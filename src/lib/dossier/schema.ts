...
export const schemaDossier = z
  .object({
    ...
    prenomNom: z
      .string()
      .trim()
      .regex(REGEX_PRENOM_NOM, MESSAGES.prenomNom),
    ...
    certification104: z.literal(true, {
      message: MESSAGES.certification104,
    }),
  ...
  .refine(
    (d) => d.portraitType !== "ACTEUR" || (d.acteurNom ?? "").trim().length > 0,
    { path: ["acteurNom"], message: MESSAGES.acteurRequis },
  );
